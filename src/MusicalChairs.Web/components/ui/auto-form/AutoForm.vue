...
const props = defineProps<{
    ...
    handleSubmit?: (event: z.infer<T>) => Promise<unknown>;
}>();
...
function handleSubmit(val: z.infer<T>) {
    if (props.handleSubmit) {
        submitting.value = true;
        props.handleSubmit(val).finally(() => (submitting.value = false));
    }
}
</script>

<template>
    <component :is="formComponent" v-bind="formComponentProps" @submit.prevent class="flex flex-col gap-2">
