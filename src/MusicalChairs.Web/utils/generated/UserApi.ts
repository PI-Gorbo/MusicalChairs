...
export const userApi: IUserApi = Remoting_buildProxy_64DC51C<IUserApi>(RemotingModule_withCredentials(true, RemotingModule_withBaseUrl("http://localhost:5000", RemotingModule_createApi())), IUserApi_$reflection());

export function login(req: LoginRequest): Promise<FSharpResult$2_$union<void, string>> {
    return startAsPromise(userApi.login(req));
}

export function me(): Promise<FSharpResult$2_$union<UserDto, string>> {
